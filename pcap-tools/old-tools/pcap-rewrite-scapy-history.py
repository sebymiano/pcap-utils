...
def parse_file_and_append(file_name, write_file, packet_history, task_idx):
    global total_tasks
...
def modify_and_write_pcap(input_file, output_file, n = 0):
    global total_tasks
    ...
    print(f"Prepare array with the list of the last {n} packets")
    packet_history = list()
    task_idx = 0
    packet_history[0] = list()
    for file, _ in file_list:
        packet_count = 0
        packet_list = list()
        task_idx += 1
        with PcapReader(file) as pcap_reader:
            # Iterate over the packets in reverse order
            for packet in reversed(pcap_reader):
                packet_list.insert(0, packet)  # Insert packet at the beginning of the list
                packet_count += 1

                if packet_count >= n:
                    break
        packet_history.append(packet_list)

    print(f"Done! Let's start rewriting the pcap files")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max(os.cpu_count(), 8), initializer=register_reporter, initargs=[reporter]) as executor:
        for file, write_file in file_list:
            task_idx += 1 
            packet_history_idx = task_idx - 1
            future_to_file[executor.submit(parse_file_and_append, copy.deepcopy(file), copy.deepcopy(write_file), copy.deepcopy(packet_history[packet_history_idx]), copy.deepcopy(task_idx))] = file
