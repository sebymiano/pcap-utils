...
def parse_file_and_append(file_name, lock, cv, task_idx, order_list, pktdump, lenght, add_payload=False):
    global total_tasks
    ...
    with PcapWriter(output_file, append=True, sync=True) as pktdump:
        with ThreadPoolExecutor(max_workers=min(os.cpu_count(), 8)) as executor:
            for file in file_list:
                task_idx += 1
                executor.submit(parse_file_and_append, copy.deepcopy(file), file_lock, cv, copy.deepcopy(task_idx), task_order_list, pktdump, lenght, add_payload)
