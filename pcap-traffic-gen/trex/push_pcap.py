...
# Add the path to the trex_client folder to sys.path
client_lib_path = os.path.join(current_dir, 'trex-core', 'scripts', 'automation', 'trex_control_plane', 'interactive')

sys.path.insert(0, client_lib_path)

# Now you can import the STLClient API
...
def inject_pcap (c, pcap_file, port, loop_count, ipg_usec, duration):
    ...
    c.reset(ports = [port])
    c.push_remote(pcap_file, ports = [port], ipg_usec = ipg_usec, speedup = 10.0, count = loop_count, duration = duration)
...
def setParserOptions():
    parser = argparse.ArgumentParser(prog="push_pcap.py")
...
def start (args):
    ...
    try:
        ...
        for i, cap in enumerate(caps, start = 1):
            ...
            injected = inject_pcap(c, cap, options.port, options.loop_count, options.ipg, 10)
